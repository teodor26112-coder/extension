...
async def flood_settings(update: Update, context: CallbackContext) -> None:
...
async def flood_status(update: Update, context: CallbackContext) -> None:
    chat = update.effective_chat
    if not chat:
        return
    db = context.application.bot_data.get("db")  # type: ignore[assignment]
...
def add_handlers(application: Application, _: "DatabaseManager") -> None:
    application.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, check_flood, block=False),
        group=1,
    )
    application.add_handler(CommandHandler("flood_config", flood_settings, block=False))
    application.add_handler(CommandHandler("flood_status", flood_status, block=False))
